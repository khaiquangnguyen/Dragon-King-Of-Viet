...
namespace ECM2.Examples.Ladders {
    public sealed class Ladder : MonoBehaviour {
        ...
        public Vector3 PathOffset = new(0f, 0f, -0.5f);
        ...
        public Vector3 ClosestPointOnPath(Vector3 position, out float pathPosition) {
            var path = topAnchorPoint - bottomAnchorPoint;
            var pathToPoint = position - bottomAnchorPoint;

            var height = Vector3.Dot(pathToPoint, path.normalized);
