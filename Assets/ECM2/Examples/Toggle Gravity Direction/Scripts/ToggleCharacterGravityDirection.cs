...
namespace ECM2.Examples.ToggleGravityDirection {
    ...
    public class ToggleCharacterGravityDirection : MonoBehaviour {
        ...
        private void RotateCharacterToGravity(float deltaTime) {
            ...
            var worldUp = -1.0f * _character.GetGravityDirection();
            var characterUp = _character.GetUpVector();

            var characterRotation = _character.GetRotation();
            var targetRotation = Quaternion.FromToRotation(characterUp, worldUp) * characterRotation;
