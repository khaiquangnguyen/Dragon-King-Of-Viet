    ...
    public class JumpAbility : MonoBehaviour {
        ...
        public virtual bool CanJump() {
            ...
            if (_jumpCount == 0) {
                ...
                var canJump = _character.IsWalking() ||
                              (_character.IsFalling() && jumpPostGroundedTime > 0.0f &&
                               _character.fallingTime < jumpPostGroundedTime);
        ...
        protected virtual Vector3 CalcJumpImpulse() {
            var worldUp = -_character.GetGravityDirection();

            var verticalSpeed = Vector3.Dot(_character.GetVelocity(), worldUp);
            var actualJumpImpulse = Mathf.Max(verticalSpeed, jumpImpulse);
        ...
        protected virtual void DoJump(float deltaTime) {
            ...
            if (_jumpButtonPressed && !IsJumping()) {
                ...
                if (jumpPreGroundedTime > 0.0f) {
                    var canJump = _jumpButtonHeldDownTime <= jumpPreGroundedTime;
                    if (!canJump)
                        return;
                }
        ...
        protected virtual void Jumping(float deltaTime) {
            ...
            if (IsJumping() && _jumpButtonPressed && jumpMaxHoldTime > 0.0f && _jumpHoldTime < jumpMaxHoldTime) {
                var actualGravity = _character.GetGravityVector();

                var actualGravityMagnitude = actualGravity.magnitude;
                var actualGravityDirection = actualGravityMagnitude > 0.0f
                    ? actualGravity / actualGravityMagnitude
                    : Vector3.zero;

                var jumpProgress = Mathf.InverseLerp(0.0f, jumpMaxHoldTime, _jumpHoldTime);
                var proportionalForce = Mathf.LerpUnclamped(actualGravityMagnitude, 0.0f, jumpProgress);

                var proportionalJumpForce = -actualGravityDirection * proportionalForce;
