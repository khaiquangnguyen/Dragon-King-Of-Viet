...
namespace ECM2 {
    ...
    public class NavMeshCharacter : MonoBehaviour {
        ...
        public virtual void MoveToDestination(Vector3 destination) {
            var worldUp = -character.GetGravityDirection();
            var toDestination2D = Vector3.ProjectOnPlane(destination - character.position, worldUp);
        ...
        protected virtual Vector3 CalcMovementDirection(Vector3 desiredVelocity) {
            var worldUp = -character.GetGravityDirection();
            var desiredVelocity2D = Vector3.ProjectOnPlane(desiredVelocity, worldUp);

            var scaledDesiredVelocity2D = desiredVelocity2D * brakingRatio;

            var minAnalogSpeed = _character.GetMinAnalogSpeed();
        ...
        protected virtual void DoPathFollowing() {
            ...
            if (agent.remainingDistance <= stoppingDistance) {
                ...
                var movementDirection = CalcMovementDirection(agent.desiredVelocity);
        ...
        /// </summary>
        protected virtual void OnMovementModeChanged(Character.MovementMode prevMovementMode,
            int prevCustomMovementMode) {
            if (!character.IsWalking() || !character.IsFalling()) StopMovement();
        }
