...
namespace ECM2 {
    ...
    public struct FindGroundResult {
        ...
        public void SetFromRaycastResult(bool hitGround, bool isWalkable, Vector3 position, float sweepDistance,
            ...
            var oldDistance = hitResult.distance;
    ...
    public sealed class CharacterMovement : MonoBehaviour {
        ...
        private Vector3 _rootTransformOffset = new(0, 0, 0);
        ...
        [Tooltip(
            "Overrides the global Physics.queriesHitTriggers to specify whether queries (raycast, spherecast, overlap tests, etc.) hit Triggers by default." +
        ...
        /// </summary>
        public delegate void CollisionResponseCallback(ref CollisionResult inCollisionResult,
            ref Vector3 characterImpulse, ref Vector3 otherImpulse);
        ...
        private Vector3 FindOpposingNormal(Vector3 sweepDirDenorm, ref RaycastHit inHit) {
            ...
            var result = inHit.normal;

            var rayOrigin = inHit.point - sweepDirDenorm;

            var rayLength = sweepDirDenorm.magnitude * 2f;
            var rayDirection = sweepDirDenorm / sweepDirDenorm.magnitude;

            if (Raycast(rayOrigin, rayDirection, rayLength, _collisionLayers, out var hitResult, kThickness))
        ...
        private static Vector3 FindBoxOpposingNormal(Vector3 sweepDirDenorm, ref RaycastHit inHit) {
            var localToWorld = inHit.transform;

            var localContactNormal = localToWorld.InverseTransformDirection(inHit.normal);
            var localTraceDirDenorm = localToWorld.InverseTransformDirection(sweepDirDenorm);

            var bestLocalNormal = localContactNormal;
            var bestOpposingDot = float.MaxValue;

            for (var i = 0; i < 3; i++)
                if (localContactNormal[i] > kKindaSmallNumber) {
                    var traceDotFaceNormal = localTraceDirDenorm[i];
        ...
        private static Vector3 FindBoxOpposingNormal(Vector3 displacement, Vector3 hitNormal, Transform hitTransform) {
            var localToWorld = hitTransform;

            var localContactNormal = localToWorld.InverseTransformDirection(hitNormal);
            var localTraceDirDenorm = localToWorld.InverseTransformDirection(displacement);
        ...
        private static Vector3 FindTerrainOpposingNormal(ref RaycastHit inHit) {
            var terrainCollider = inHit.collider as TerrainCollider;

            if (terrainCollider != null) {
                var localPoint = terrainCollider.transform.InverseTransformPoint(inHit.point);

                var terrainData = terrainCollider.terrainData;

                var interpolatedNormal = terrainData.GetInterpolatedNormal(localPoint.x / terrainData.size.x,
        ...
        private Vector3 FindGeomOpposingNormal(Vector3 sweepDirDenorm, ref RaycastHit inHit) {
            ...
            if (inHit.collider is BoxCollider _) return FindBoxOpposingNormal(sweepDirDenorm, ref inHit);
            ...
            if (inHit.collider is MeshCollider nonConvexMeshCollider && !nonConvexMeshCollider.convex) {
                var sharedMesh = nonConvexMeshCollider.sharedMesh;
            ...
            if (inHit.collider is TerrainCollider) return FindTerrainOpposingNormal(ref inHit);
        ...
        private static Vector3 ApplyVelocityBraking(Vector3 currentVelocity, float friction, float deceleration,
            float deltaTime) {
            // If no friction or no deceleration, return

            var isZeroFriction = friction == 0.0f;
            var isZeroBraking = deceleration == 0.0f;
            ...
            var revAcceleration = isZeroBraking ? Vector3.zero : -deceleration * currentVelocity.normalized;
            ...
            var sqrSpeed = currentVelocity.sqrMagnitude;
        ...
        private static Vector3 CalcVelocity(Vector3 currentVelocity, Vector3 desiredVelocity, float maxSpeed,
            ...
            var analogInputModifier = ComputeAnalogInputModifier(desiredVelocity, maxSpeed);
            var requestedAcceleration = acceleration * analogInputModifier * desiredMoveDirection;

            // Actual max speed (factoring analog input)

            var actualMaxSpeed = Mathf.Max(0.0f, maxSpeed * analogInputModifier);

            // Friction
            // Only apply braking if there is no input acceleration,
            // or we are over our max speed and need to slow down to it

            var isZeroAcceleration = requestedAcceleration.isZero();
            var isVelocityOverMax = currentVelocity.isExceeding(actualMaxSpeed);
            ...
            if (isZeroAcceleration || isVelocityOverMax) {
                ...
                currentVelocity -= (currentVelocity - desiredMoveDirection * currentVelocity.magnitude) *
            ...
            if (!isZeroAcceleration) {
                var newMaxSpeed = currentVelocity.isExceeding(actualMaxSpeed)
        ...
        private static void MakeCapsule(float radius, float height, out Vector3 center, out Vector3 bottomCenter,
            ...
            var sideHeight = height - radius * 2.0f;
        ...
        private HitLocation ComputeHitLocation(Vector3 inNormal) {
            var verticalComponent = inNormal.dot(_characterUp);
        ...
        private bool IsWalkable(Collider inCollider, Vector3 inNormal) {
            ...
            if (collisionBehaviourCallback != null) {
                var collisionBehaviour = collisionBehaviourCallback.Invoke(inCollider);
            ...
            var actualSlopeLimit = _minSlopeLimit;
        ...
        private Vector3 ComputeBlockingNormal(Vector3 inNormal, bool isWalkable) {
            if ((isGrounded || _hasLanded) && !isWalkable) {
                var actualGroundNormal = _hasLanded ? _foundGround.normal : _currentGround.normal;

                var forward = actualGroundNormal.perpendicularTo(inNormal);
                var blockingNormal = forward.perpendicularTo(_characterUp);
        ...
        private bool ComputeInflatedMTD(Vector3 characterPosition, Quaternion characterRotation, float mtdInflation,
            ...
            var mtdResult = Physics.ComputePenetration(_capsuleCollider, characterPosition, characterRotation,
                hitCollider, hitTransform.position, hitTransform.rotation, out var recoverDirection,
                out var recoverDistance);
        ...
        /// </summary>
        private bool ComputeMTD(Vector3 characterPosition, Quaternion characterRotation, Collider hitCollider,
        ...
        /// </summary>
        private void
            ResolveOverlaps(DepenetrationBehaviour depenetrationBehaviour = DepenetrationBehaviour.IgnoreNone) {
            if (!detectCollisions)
                return;

            var ignoreStatic = (depenetrationBehaviour & DepenetrationBehaviour.IgnoreStatic) != 0;
            var ignoreDynamic = (depenetrationBehaviour & DepenetrationBehaviour.IgnoreDynamic) != 0;
            var ignoreKinematic = (depenetrationBehaviour & DepenetrationBehaviour.IgnoreKinematic) != 0;

            for (var i = 0; i < _advanced.maxDepenetrationIterations; i++) {
                var top = updatedPosition + _transformedCapsuleTopCenter;
                var bottom = updatedPosition + _transformedCapsuleBottomCenter;

                var overlapCount = Physics.OverlapCapsuleNonAlloc(bottom, top, _radius, _overlaps, _collisionLayers,
                    triggerInteraction);
                if (overlapCount == 0)
                    break;
                ...
                for (var j = 0; j < overlapCount; j++) {
                    ...
                    if (attachedRigidbody) {
                        var isKinematic = attachedRigidbody.isKinematic;
                    ...
                    if (ComputeMTD(updatedPosition, updatedRotation, overlappedCollider, overlappedCollider.transform,
                            out var recoverDirection, out var recoverDistance)) {
                        recoverDirection = ConstrainDirectionToPlane(recoverDirection);

                        var hitLocation = ComputeHitLocation(recoverDirection);

                        var isWalkable = IsWalkable(overlappedCollider, recoverDirection);

                        var impactNormal = ComputeBlockingNormal(recoverDirection, isWalkable);
        ...
        public int OverlapTest(Vector3 characterPosition, Quaternion characterRotation, float testRadius,
            ...
            var top = characterPosition + characterRotation * topCenter;
            var bottom = characterPosition + characterRotation * bottomCenter;
        ...
        public bool IsWithinEdgeTolerance(Vector3 characterPosition, Vector3 inPoint, float testRadius) {
            var distFromCenterSq = (inPoint - characterPosition).projectedOnPlane(_characterUp).sqrMagnitude;

            var reducedRadius = Mathf.Max(kSweepEdgeRejectDistance + kKindaSmallNumber,
        ...
        private bool IsValidLandingSpot(Vector3 characterPosition, ref CollisionResult inCollision) {
            ...
            FindGround(characterPosition, out var groundResult);
            {
        ...
        public bool Raycast(Vector3 origin, Vector3 direction, float distance, int layerMask, out RaycastHit hitResult,
            ...
            var rawHitCount = thickness == 0.0f
        ...
        private bool CapsuleCast(Vector3 characterPosition, float castRadius, Vector3 castDirection, float castDistance,
            ...
            var top = characterPosition + _transformedCapsuleTopCenter;
            var bottom = characterPosition + _transformedCapsuleBottomCenter;

            var rawHitCount = Physics.CapsuleCastNonAlloc(bottom, top, castRadius, castDirection, _hits, castDistance,
                layerMask, triggerInteraction);
        ...
        private bool CapsuleCastEx(Vector3 characterPosition, float castRadius, Vector3 castDirection,
            float castDistance, int layerMask,
            out RaycastHit hitResult, out bool startPenetrating, out Vector3 recoverDirection,
            ...
            for (var i = 0; i < rawHitCount; i++) {
                ...
                var isOverlapping = hit.distance <= 0.0f;
                if (isOverlapping)
                    if (ComputeMTD(characterPosition, updatedRotation, hit.collider, hit.collider.transform,
                            out var mtdDirection, out var mtdDistance)) {
                        mtdDirection = ConstrainDirectionToPlane(mtdDirection);

                        var hitLocation = ComputeHitLocation(mtdDirection);
                        ...
                        var impactNormal = ComputeBlockingNormal(mtdDirection, IsWalkable(hit.collider, mtdDirection));
            ...
            if (rawHitCount > 2) SortArray(_hits, rawHitCount);

            var mostOpposingDot = Mathf.Infinity;
            ...
            for (var i = 0; i < rawHitCount; i++) {
                ...
                var isOverlapping = hit.distance <= 0.0f && !hit.point.isZero();
                if (isOverlapping) {
                    // Overlaps

                    var movementDotNormal = Vector3.Dot(castDirection, hit.normal);
                    ...
                    if (ignoreNonBlockingOverlaps) {
                        ...
                        var isMovingOut = movementDotNormal > 0.0f;
                        if (isMovingOut)
                            continue;
                    }
        ...
        private bool SweepTest(Vector3 sweepOrigin, float sweepRadius, Vector3 sweepDirection, float sweepDistance,
            ...
            var outerCapsuleRadius = sweepRadius + kContactOffset;
            ...
            var foundBlockingHit = innerCapsuleHit || outerCapsuleHit;
            if (!foundBlockingHit)
                return false;
        ...
        private bool SweepTestEx(Vector3 sweepOrigin, float sweepRadius, Vector3 sweepDirection, float sweepDistance,
            ...
            var innerCapsuleHit =
                CapsuleCastEx(sweepOrigin, sweepRadius, sweepDirection, sweepDistance + sweepRadius, sweepLayerMask,
                    out var innerCapsuleHitResult, out startPenetrating, out recoverDirection, out recoverDistance,
                    ignoreBlockingOverlaps) && innerCapsuleHitResult.distance <= sweepDistance;
            ...
            var outerCapsuleHit =
                CapsuleCast(sweepOrigin, outerCapsuleRadius, sweepDirection, sweepDistance + outerCapsuleRadius,
                    sweepLayerMask,
                    out var outerCapsuleHitResult, out _) && outerCapsuleHitResult.distance <= sweepDistance;
        ...
        private bool ResolvePenetration(Vector3 displacement, Vector3 proposedAdjustment) {
            var adjustment = ConstrainVectorToPlane(proposedAdjustment);
            ...
            if (!(OverlapTest(updatedPosition + adjustment, updatedRotation, _radius + kOverlapInflation, _height,
                    _collisionLayers, _overlaps, triggerInteraction) > 0)) {
                ...
                var lastPosition = updatedPosition;
                ...
                var hit = CapsuleCastEx(updatedPosition, _radius, adjustment.normalized, adjustment.magnitude,
                    _collisionLayers,
                    out var sweepHitResult, out var startPenetrating, out var recoverDirection, out var recoverDistance,
                    true);

                if (!hit)
                    updatedPosition += adjustment;
                else
                    updatedPosition += adjustment.normalized *
                                       Mathf.Max(sweepHitResult.distance - kSmallContactOffset, 0.0f);

                // Still stuck?

                var moved = updatedPosition != lastPosition;
                if (!moved && startPenetrating) {
                    // Combine two MTD results to get a new direction that gets out of multiple surfaces.

                    var secondMTD = recoverDirection * (recoverDistance + kContactOffset + kPenetrationOffset);
                    ...
                    if (secondMTD != adjustment && !combinedMTD.isZero()) {
                        ...
                        if (!hit)
                            ...
                            updatedPosition += combinedMTD.normalized *
                                               Mathf.Max(sweepHitResult.distance - kSmallContactOffset, 0.0f);
                ...
                if (!moved) {
                    ...
                    var moveDelta = ConstrainVectorToPlane(displacement);
                    if (!moveDelta.isZero()) {
                        lastPosition = updatedPosition;

                        var newAdjustment = adjustment + moveDelta;
                        ...
                        if (!hit)
                            ...
                            updatedPosition += newAdjustment.normalized *
                                               Mathf.Max(sweepHitResult.distance - kSmallContactOffset, 0.0f);
                        ...
                        if (!moved && Vector3.Dot(moveDelta, adjustment) > 0.0f) {
                            ...
                            if (!hit)
                                ...
                                updatedPosition += moveDelta.normalized *
                                                   Mathf.Max(sweepHitResult.distance - kSmallContactOffset, 0.0f);
        ...
        private bool MovementSweepTest(Vector3 characterPosition, Vector3 inVelocity, Vector3 displacement,
            ...
            var sweepOrigin = characterPosition;
            var sweepDirection = displacement.normalized;

            var sweepRadius = _radius;
            var sweepDistance = displacement.magnitude;

            int sweepLayerMask = _collisionLayers;

            var hit = SweepTestEx(sweepOrigin, sweepRadius, sweepDirection, sweepDistance, sweepLayerMask,
                out var hitResult, out var startPenetrating, out var recoverDirection, out var recoverDistance);

            if (startPenetrating) {
                // Handle initial penetrations

                var requestedAdjustment = recoverDirection * (recoverDistance + kContactOffset + kPenetrationOffset);
            ...
            var hitLocation = ComputeHitLocation(hitResult.normal);

            var displacementToHit = sweepDirection * hitResult.distance;
            var remainingDisplacement = displacement - displacementToHit;

            var hitPosition = sweepOrigin + displacementToHit;

            var surfaceNormal = hitResult.normal;

            var isWalkable = false;
            var hitGround = hitLocation == HitLocation.Below;
        ...
        private Vector3 HandleSlopeBoosting(Vector3 slideResult, Vector3 displacement, Vector3 inNormal) {
            ...
            var yResult = Vector3.Dot(result, _characterUp);
            if (yResult > 0.0f) {
                // Don't move any higher than we originally intended.

                var yLimit = Vector3.Dot(displacement, _characterUp);
                if (yResult - yLimit > kKindaSmallNumber) {
                    ...
                    var lateralRemainder = (slideResult - result).projectedOnPlane(_characterUp);
                    var lateralNormal = inNormal.projectedOnPlane(_characterUp).normalized;
                    var adjust = lateralRemainder.projectedOnPlane(lateralNormal);
        ...
        private Vector3 ComputeSlideVector(Vector3 displacement, Vector3 inNormal, bool isWalkable) {
            if (isGrounded) {
                if (isWalkable) {
                    ...
                    var right = inNormal.perpendicularTo(groundNormal);
                    var up = right.perpendicularTo(inNormal);
                ...
                if (isWalkable) {
                    ...
                    var slideResult = displacement.projectedOnPlane(inNormal);
        ...
        private int SlideAlongSurface(int iteration, Vector3 inputDisplacement, ref Vector3 inVelocity,
            ...
            if (useFlatTop && inHit.hitLocation == HitLocation.Above) {
                var surfaceNormal = FindBoxOpposingNormal(displacement, inHit.normal, inHit.transform);
            ...
            if (inHit.isWalkable && isConstrainedToGround) {
                ...
                if (iteration == 0) {
                    ...
                    var crease = prevNormal.perpendicularTo(inHit.normal);

                    var oVel = inputDisplacement.projectedOnPlane(crease);

                    var nVel = ComputeSlideVector(displacement, inHit.normal, inHit.isWalkable);
        ...
        private void PerformMovement(float deltaTime) {
            ...
            var depenetrationFlags = !enablePhysicsInteraction
            ...
            var inputDisplacement = displacement;
            ...
            for (var i = 0; i < _collisionCount; i++) {
                ref var collisionResult = ref _collisionResults[i];

                var opposesMovement = displacement.dot(collisionResult.normal) < 0.0f;
                if (!opposesMovement)
                    continue;
                ...
                if (isConstrainedToGround && !isOnWalkableGround) {
                    if (IsValidLandingSpot(updatedPosition, ref collisionResult)) {
                        ...
                        if (collisionResult.hitLocation == HitLocation.Below) {
                            FindGround(updatedPosition, out var groundResult);
            ...
            var maxSlideCount = _advanced.maxMovementIterations;
            while (detectCollisions && maxSlideCount-- > 0 &&
                   displacement.sqrMagnitude > _advanced.minMoveDistanceSqr) {
                var collided = MovementSweepTest(updatedPosition, _velocity, displacement,
                ...
                if (isGrounded && !collisionResult.isWalkable)
                    if (CanStepUp(collisionResult.collider) &&
                        StepUp(ref collisionResult, out var stepResult)) {
                ...
                if (isConstrainedToGround && !isOnWalkableGround) {
                    ...
                    if (!_hasLanded && collisionResult.hitLocation == HitLocation.Below) {
                        var sweepDistance = collisionResult.hitResult.distance;
                        var surfaceNormal = collisionResult.surfaceNormal;
        ...
        private bool CanPerchOn(Collider otherCollider) {
            ...
            if (collisionBehaviourCallback != null) {
                var collisionBehaviour = collisionBehaviourCallback.Invoke(otherCollider);
        ...
        private bool ShouldComputePerchResult(Vector3 characterPosition, ref RaycastHit inHit) {
            ...
            if (GetPerchRadiusThreshold() <= kSweepEdgeRejectDistance) return false;

            var distFromCenterSq = (inHit.point - characterPosition).projectedOnPlane(_characterUp).sqrMagnitude;
            var standOnEdgeRadius = GetValidPerchRadius(inHit.collider);
        ...
        private bool CapsuleCast(Vector3 point1, Vector3 point2, float castRadius, Vector3 castDirection,
            ...
            var rawHitCount = Physics.CapsuleCastNonAlloc(point1, point2, castRadius, castDirection, _hits,
        ...
        private bool BoxCast(Vector3 center, Vector3 halfExtents, Quaternion orientation, Vector3 castDirection,
            ...
            var rawHitCount = Physics.BoxCastNonAlloc(center, halfExtents, castDirection, _hits, orientation,
                castDistance, castLayerMask, triggerInteraction);
        ...
        private bool GroundSweepTest(Vector3 characterPosition, float capsuleRadius, float capsuleHalfHeight,
            ...
            if (!useFlatBaseForGroundChecks) {
                var characterCenter = characterPosition + _transformedCapsuleCenter;

                var point1 = characterCenter - _characterUp * (capsuleHalfHeight - capsuleRadius);
                var point2 = characterCenter + _characterUp * (capsuleHalfHeight - capsuleRadius);

                var sweepDirection = -1.0f * _characterUp;
                ...
                var center = characterPosition + _transformedCapsuleCenter;
                var halfExtents = new Vector3(capsuleRadius * 0.707f, capsuleHalfHeight, capsuleRadius * 0.707f);

                var sweepOrientation = rotation * Quaternion.Euler(0f, -rotation.eulerAngles.y, 0f);
                var sweepDirection = -1.0f * _characterUp;

                var sweepLayerMask = _collisionLayers;
        ...
        public void ComputeGroundDistance(Vector3 characterPosition, float sweepRadius, float sweepDistance,
            ...
            var characterRadius = _radius;
            var characterHeight = _height;
            var characterHalfHeight = characterHeight * 0.5f;
            ...
            if (sweepDistance > 0.0f && sweepRadius > 0.0f) {
                ...
                var shrinkHeight = (characterHalfHeight - characterRadius) * (1.0f - kShrinkScale);

                var capsuleRadius = sweepRadius;
                var capsuleHalfHeight = characterHalfHeight - shrinkHeight;

                var actualSweepDistance = sweepDistance + shrinkHeight;
                ...
                if (foundGround || startPenetrating) {
                    ...
                    if (startPenetrating || !IsWithinEdgeTolerance(characterPosition, hitResult.point, capsuleRadius)) {
                        ...
                        capsuleRadius = Mathf.Max(0.0011f,
                            capsuleRadius - kSweepEdgeRejectDistance - kKindaSmallNumber);
                    ...
                    if (foundGround && !startPenetrating) {
                        ...
                        var maxPenetrationAdjust = Mathf.Max(kMaxGroundDistance, characterRadius);
                        var sweepResult = Mathf.Max(-maxPenetrationAdjust, hitResult.distance - shrinkHeight);

                        var sweepDirection = -1.0f * _characterUp;
                        var hitPosition = characterPosition + sweepDirection * sweepResult;

                        var surfaceNormal = hitResult.normal;

                        var isWalkable = false;
                        var hitGround = sweepResult <= sweepDistance &&
            ...
            if (castDistance > 0.0f) {
                var rayOrigin = characterPosition + _transformedCapsuleCenter;
                var rayDirection = -1.0f * _characterUp;

                var shrinkHeight = characterHalfHeight;
                var rayLength = castDistance + shrinkHeight;

                foundGround = Raycast(rayOrigin, rayDirection, rayLength, _collisionLayers, out var hitResult);
                ...
                if (foundGround && hitResult.distance > 0.0f) {
                    ...
                    var MaxPenetrationAdjust = Mathf.Max(kMaxGroundDistance, characterRadius);
                    var castResult = Mathf.Max(-MaxPenetrationAdjust, hitResult.distance - shrinkHeight);
        ...
        private bool ComputePerchResult(Vector3 characterPosition, float testRadius, float inMaxGroundDistance,
            ...
            var inHitAboveBase = Mathf.Max(0.0f, Vector3.Dot(inHit.point - characterPosition, _characterUp));
            var perchCastDist = Mathf.Max(0.0f, inMaxGroundDistance - inHitAboveBase);
            var perchSweepDist = Mathf.Max(0.0f, inMaxGroundDistance);

            var actualSweepDist = perchSweepDist + _radius;
        ...
        public void FindGround(Vector3 characterPosition, out FindGroundResult outGroundResult) {
            ...
            var heightCheckAdjust = isGrounded ? kMaxGroundDistance + kKindaSmallNumber : -kMaxGroundDistance;
            var sweepDistance = Mathf.Max(kMaxGroundDistance, stepOffset + heightCheckAdjust);
            ...
            if (outGroundResult.hitGround && !outGroundResult.isRaycastResult) {
                var positionOnGround = outGroundResult.position;

                if (ShouldComputePerchResult(positionOnGround, ref outGroundResult.hitResult)) {
                    var maxPerchGroundDistance = sweepDistance;
                    if (isGrounded)
                        maxPerchGroundDistance += perchAdditionalHeight;

                    var validPerchRadius = GetValidPerchRadius(outGroundResult.collider);

                    if (ComputePerchResult(positionOnGround, validPerchRadius, maxPerchGroundDistance,
                            ref outGroundResult.hitResult, out var perchGroundResult)) {
                        ...
                        var moveUpDist = kAvgGroundDistance - outGroundResult.groundDistance;
                        ...
                        if (!outGroundResult.isWalkableGround) {
                            ...
                            var groundDistance = outGroundResult.groundDistance;
                            var raycastDistance = Mathf.Max(kMinGroundDistance, groundDistance);
        ...
        private void AdjustGroundHeight() {
            ...
            var lastGroundDistance = _currentGround.groundDistance;
            ...
            if (lastGroundDistance < kMinGroundDistance || lastGroundDistance > kMaxGroundDistance) {
                var initialY = Vector3.Dot(updatedPosition, _characterUp);
                var moveDistance = kAvgGroundDistance - lastGroundDistance;

                var displacement = _characterUp * moveDistance;

                var sweepOrigin = updatedPosition;
                var sweepDirection = displacement.normalized;
                ...
                var hit = SweepTestEx(sweepOrigin, sweepRadius, sweepDirection, sweepDistance, sweepLayerMask,
                    out var hitResult, out var startPenetrating, out _, out _, true);
                ...
                if (!hit && !startPenetrating) {
                    ...
                    var currentY = Vector3.Dot(updatedPosition, _characterUp);
        ...
        private bool StepUp(ref CollisionResult inCollision, out CollisionResult stepResult) {
            ...
            var characterInitialGroundPositionY = Vector3.Dot(inCollision.position, _characterUp);
            var groundPointY = characterInitialGroundPositionY;

            var actualGroundDistance = Mathf.Max(0.0f, _currentGround.GetDistanceToGround());
            characterInitialGroundPositionY -= actualGroundDistance;

            var stepTravelUpHeight = Mathf.Max(0.0f, stepOffset - actualGroundDistance);
            var stepTravelDownHeight = stepOffset + kMaxGroundDistance * 2.0f;
            ...
            var initialImpactY = Vector3.Dot(inCollision.point, _characterUp);
            if (initialImpactY <= characterInitialGroundPositionY)
                return false;

            // Step up, treat as vertical wall

            var sweepOrigin = inCollision.position;
            var sweepDirection = _characterUp;

            var sweepRadius = _radius;
            var sweepDistance = stepTravelUpHeight;

            int sweepLayerMask = _collisionLayers;

            var foundBlockingHit = SweepTest(sweepOrigin, sweepRadius, sweepDirection, sweepDistance, sweepLayerMask,
                out var hitResult, out var startPenetrating);
            ...
            var displacement = inCollision.remainingDisplacement;
            var displacement2D = ConstrainVectorToPlane(Vector3.ProjectOnPlane(displacement, _characterUp));
            ...
            var deltaY = Vector3.Dot(hitResult.point, _characterUp) - groundPointY;
            if (deltaY > stepOffset)
                return false;

            // Is position on step clear ?

            var positionOnStep = sweepOrigin + sweepDirection * hitResult.distance;

            if (OverlapTest(positionOnStep, updatedRotation, _radius, _height, _collisionLayers, _overlaps,
                    triggerInteraction) > 0)
                return false;

            // Reject unwalkable surface normals here.

            var surfaceNormal = FindGeomOpposingNormal(sweepDirection * sweepDistance, ref hitResult);

            var isWalkable = IsWalkable(hitResult.collider, surfaceNormal);
            if (!isWalkable) {
                // Reject if normal opposes movement direction.

                var normalTowardsMe = Vector3.Dot(displacement, surfaceNormal) < 0.0f;
                if (normalTowardsMe)
                    return false;
        ...
        private void MoveAndSlide(Vector3 displacement) {
            ...
            while (maxSlideCount-- > 0 && displacement.sqrMagnitude > _advanced.minMoveDistanceSqr) {
                var collided = MovementSweepTest(updatedPosition, default, displacement, out var collisionResult);
                if (!collided)
                    break;
                ...
                iteration = SlideAlongSurface(iteration, inputDisplacement, ref displacement, ref collisionResult,
                    ref prevNormal);
        ...
        private void UpdateCurrentPlatform() {
            ...
            if (_movingPlatform.platform != null) {
                var platformTransform = _movingPlatform.platform.transform;
        ...
        private void UpdatePlatformMovement(float deltaTime) {
            var lastPlatformVelocity = _movingPlatform.platformVelocity;
            ...
            if (!_movingPlatform.platform) {
                ...
                var newPositionOnPlatform = platformTransform.TransformPoint(_movingPlatform.localPosition);
                var deltaPosition = newPositionOnPlatform - _movingPlatform.position;
                ...
                if (impartPlatformRotation) {
                    var newRotationOnPlatform = platformTransform.rotation * _movingPlatform.localRotation;
                    var deltaRotation = newRotationOnPlatform * Quaternion.Inverse(_movingPlatform.rotation);
            ...
            if (impartPlatformVelocity) {
                var impartedPlatformVelocity = Vector3.zero;
        ...
        private void ComputeDynamicCollisionResponse(ref CollisionResult inCollisionResult,
            ...
            var otherRigidbody = inCollisionResult.rigidbody;
            ...
            var normal = inCollisionResult.normal;

            var velocityDotNormal = Vector3.Dot(inCollisionResult.velocity, normal);
            var otherVelocityDotNormal = Vector3.Dot(inCollisionResult.otherVelocity, normal);

            if (velocityDotNormal < 0.0f)
                characterImpulse += velocityDotNormal * normal;

            if (otherVelocityDotNormal > velocityDotNormal) {
                var relVel = (otherVelocityDotNormal - velocityDotNormal) * normal;
        ...
        private void ResolveDynamicCollisions() {
            ...
            for (var i = 0; i < _collisionCount; i++) {
                ...
                var otherRigidbody = collisionResult.rigidbody;
                if (otherRigidbody == null)
                    continue;

                ComputeDynamicCollisionResponse(ref collisionResult, out var characterImpulse, out var otherImpulse);
        ...
        /// </summary>
        public void AddExplosionForce(float strength, Vector3 origin, float radius,
            ForceMode forceMode = ForceMode.Force) {
            var delta = worldCenter - origin;
        ...
        public void LaunchCharacter(Vector3 launchVelocity, bool overrideVerticalVelocity = false,
            ...
            var finalVelocity = launchVelocity;
        ...
        public CollisionFlags SimpleMove(Vector3 desiredVelocity, float maxSpeed, float acceleration,
            float deceleration, float friction, float brakingFriction, Vector3 gravity, bool onlyHorizontal,
            float deltaTime) {
            if (isGrounded) {
                // Calc new velocity
                ...
                var worldUp = -1.0f * gravity.normalized;
                var v = onlyHorizontal ? velocity.projectedOnPlane(worldUp) : velocity;
                ...
                if (isOnGround) {
                    ...
                    var actualGroundNormal = groundNormal;
                ...
                v = CalcVelocity(v, desiredVelocity, maxSpeed, acceleration, deceleration, friction, brakingFriction,
                    deltaTime);
        ...
        private static void DrawDisc(Vector3 _pos, Quaternion _rot, float _radius, Color _color = default,
            ...
            var mtx = Matrix4x4.TRS(_pos, _rot, UnityEditor.Handles.matrix.lossyScale);
