    ...
    public class DataTableDrawer : PropertyDrawerBase {
        public override VisualElement CreatePropertyGUI(SerializedProperty property) {
            ...
            if (property.propertyType != SerializedPropertyType.Generic) {
                var errorBox =
                    new HelpBox(
                        "The DataTableAttribute can only be attached to serialized structs or classes and collections containing them",
            ...
            while (serializedProperty.NextVisible(true) && serializedProperty.depth > initialDepth) {
                if (serializedProperty.propertyType is SerializedPropertyType.Generic or SerializedPropertyType.Vector4
                    or SerializedPropertyType.ArraySize) {
                    var errorBox = new HelpBox("Collection, UnityEvent and Serialized object types are not supported",
                        HelpBoxMessageType.Error);
                    root.Add(errorBox);
                    break;
                }

                if (serializedProperty.depth >=
                    initialDepth +
                    2) // Skip the X Y Z properties that are inside Vectors since we draw the vector field ourself
                    continue;
