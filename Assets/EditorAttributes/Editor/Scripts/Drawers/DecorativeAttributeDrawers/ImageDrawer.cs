    ...
    public class ImageDrawer : PropertyDrawerBase {
        public override VisualElement CreatePropertyGUI(SerializedProperty property) {
            ...
            UpdateVisualElement(root, () => {
                ...
                if (texture == null) {
                    errorBox.text =
                        "The image asset could not be found make sure you inputted the correct filepath to a image asset";
                    return;
                }

                RemoveElement(root, errorBox);

                var imageWidth = imageAttribute.ImageWidth == 0f
                    ? GetTextureSize(texture).x
                    : imageAttribute.ImageWidth;
                var imageHeight = imageAttribute.ImageHeight == 0f
                    ? GetTextureSize(texture).y
                    : imageAttribute.ImageHeight;
