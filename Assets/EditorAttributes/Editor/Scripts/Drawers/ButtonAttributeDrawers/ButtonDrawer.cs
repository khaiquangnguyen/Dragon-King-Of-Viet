...
namespace EditorAttributes.Editor {
    public class ButtonDrawer {
        ...
        internal static VisualElement DrawButton(MethodInfo function, ButtonAttribute buttonAttribute,
            Dictionary<MethodInfo, bool> foldouts, Dictionary<MethodInfo, object[]> parameterValues, object target) {
            ...
            if (functionParameters.Length > 0) {
                ...
                var button = MakeButton(function, buttonAttribute, () => {
                    ...
                    for (int i = 0; i < functionParameters.Length; i++)
                        paramValueList[i] = ConvertParameterValue(functionParameters[i].ParameterType,
                ...
                for (int i = 0; i < functionParameters.Length; i++) {
                    ...
                    var field = DrawParameterField(parameter.ParameterType, parameter.Name,
                        parameterValues[function][i]);
                    ...
                    int index = i;
                    RegisterParameterFieldValueChangedCallback(field, parameter.ParameterType,
        ...
        private static VisualElement MakeButton(MethodInfo function, ButtonAttribute buttonAttribute,
            Action buttonLogic) {
            var buttonLabel = string.IsNullOrWhiteSpace(buttonAttribute.ButtonLabel)
                ? function.Name
                : buttonAttribute.ButtonLabel;

            if (buttonAttribute.IsRepetable) {
                var repeatButton =
                    new RepeatButton(buttonLogic, buttonAttribute.PressDelay, buttonAttribute.RepetitionInterval)
                        { text = buttonLabel };
        ...
        internal static void SaveParamsData(MethodInfo[] functions, object target,
            Dictionary<MethodInfo, bool> foldouts, Dictionary<MethodInfo, object[]> parameterValues) {
        ...
        internal static void LoadParamsData(MethodInfo[] functions, object target,
            ref Dictionary<MethodInfo, bool> foldouts, ref Dictionary<MethodInfo, object[]> parameterValues) {
        ...
        internal static string GetFunctionID(MethodInfo function, object target) =>
            $"{target}_{function.Name}_{string.Join("_", function.GetParameters().Select(param => param.ParameterType.Name))}";
        ...
        internal static VisualElement DrawParameterField(Type fieldType, string fieldName, object fieldValue) {
            ...
            if (fieldType == typeof(string)) {
                ...
                return new UnsignedIntegerField(fieldName)
                    { value = (uint)ConvertParameterValue(fieldType, fieldValue) };
            }
                ...
                return new Vector2IntField(fieldName)
                    { value = (Vector2Int)ConvertParameterValue(fieldType, fieldValue) };
            }
                ...
                return new Vector3IntField(fieldName)
                    { value = (Vector3Int)ConvertParameterValue(fieldType, fieldValue) };
            }
                ...
                return new CurveField(fieldName)
                    { value = (AnimationCurve)ConvertParameterValue(fieldType, fieldValue) };
            }
                ...
                return new BoundsIntField(fieldName)
                    { value = (BoundsInt)ConvertParameterValue(fieldType, fieldValue) };
            }
            else {
        ...
        private static void RegisterParameterFieldValueChangedCallback(VisualElement field, Type parameterType,
